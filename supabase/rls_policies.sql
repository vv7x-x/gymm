...
-- and a trigger that auto-creates a profile row on user signup.
-- ============================================================
...
-- 3. Helper function to check if user is staff or admin
...
CREATE POLICY "Staff read attendance" ON public.attendance FOR SELECT USING (is_staff());
CREATE POLICY "Staff insert attendance" ON public.attendance FOR INSERT WITH CHECK (is_staff());

-- settings
ALTER TABLE public.settings ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Staff read settings" ON public.settings FOR SELECT USING (is_staff());
